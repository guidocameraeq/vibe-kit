...
> te dice qué decir, dónde, y qué esperar. Actualizada 2026-07-11 (v2.1).
...
|---|---|---|
| **La máquina** (esta PC, la del SAAS, cualquiera) | El Equipador (`/arquitecto-skills`) | Al estrenar una PC o actualizar skills |
...
Se instala solo, te pregunta lo mínimo, y al final te ofrece equipar la máquina con el menú
curado. Sin git: el zip de `Desktop\Arquitecto en otras PCs\`.
Para actualizar una PC que ya lo tiene: `git pull` + el mismo prompt (detecta versiones).
...
*"qué skills tengo"* (censo), *"actualizá las skills"* (update con diff), *"agregá tal skill
al menú"* (la investiga primero — nada entra sin curaduría).
...
commits. Para el "por qué" de cualquier cosa: `docs/DECISIONS.md` (12 ADRs) y
