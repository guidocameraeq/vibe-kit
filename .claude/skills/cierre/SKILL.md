---
name: cierre
description: Cierre de sesión del repo vibe-kit — sincronizar kit a ~/.claude con diff verificado, docs al día (handoff/ADR/REJ), commit + push. Usar cuando Guido dice "cierre", "cerrá la sesión", "guardá todo" o termina una misión en este repo. Modo "cierre parcial" si hay que compactar a mitad de trabajo.
---
...
3. **Memoria**: si cambió un hecho estructural del proyecto (release, pieza nueva, regla
   nueva), actualizar la memoria persistente.
4. **Commit + push**: mensaje descriptivo en español. Si la máquina no tiene identidad de
...
5. **Cierre**: resumen de 3 líneas + "chat listo para descartar".

## Modo `cierre parcial` (emergencia pre-compactación)

Contexto lleno a mitad de misión → SOLO el paso 2a (handoff con el trabajo a medias y el
