...
- **Dos disparadores:**
  ...
  - **Proactivo (checklist FIJA, SIEMPRE, aunque el código calle)** — el silencio del código NO exime la pregunta: (1) ¿hay backups de la base/datos y dónde viven?; (2) ¿la base tiene control de acceso por fila (RLS) o equivalente?; (3) ¿la base es compartida con otras apps?; (4) ¿los tokens/credenciales que usa siguen vigentes? Son los hechos que el reporte de campo mostró como los de más valor y que el código no sabe.
...
2. **[Fórmula, no negativo]** Todo enunciado sobre seguridad/continuidad usa "no se encontró X en el código — confirmar", NUNCA un absoluto ("no hay X" ni parafraseado). **Test:** repo sin backups en el código → el artefacto usa la fórmula condicional o lo marca duda; cero absolutos negativos sobre esos temas.
3. **[Checklist proactiva]** El motor SIEMPRE plantea las 4 dudas de continuidad/acceso (backups · RLS/control de acceso · base compartida · vigencia de tokens), aunque el código calle. **Test:** en cualquier repo, esas 4 aparecen como preguntas.
